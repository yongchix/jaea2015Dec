...
public:
	...
	/* by Yongchi Xiao
	 * I decided to add two functions below, 
	 * one for integral over the tail of a signal
	 * the other one for deducing PSD value
	 */ 
	...
	double GetDiffMean(unsigned int lo, unsigned int hi);
	bool   ScanPileup(unsigned int lo,unsigned int hi);
	double DoQDCSimple(unsigned int lo,unsigned int hi);
	double DeduceRegression(unsigned int lo,unsigned int hi,unsigned int mode);
